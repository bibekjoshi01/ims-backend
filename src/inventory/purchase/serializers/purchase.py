...
from src.custom_lib.utils.date_converter import date_bs_to_date_ad_converter
from src.inventory.utils.generate_purchase_no import generate_unique_purchase_number
from src.custom_lib.functions.fiscal_session import get_current_fiscal_session_detail
from src.custom_lib.functions.core_function import (
    get_current_time_stamp_and_user_for_db_op,
)
...
class InvPurchaseCreateSerializer(serializers.ModelSerializer):
    remarks = serializers.CharField(max_length=150, allow_blank=True, required=False)
    supplier = serializers.PrimaryKeyRelatedField(
        queryset=InvSupplier.objects.filter(user__is_active=True)
    )

    bill_date_bs = serializers.CharField(
        max_length=10, validators=[validate_date_bs], allow_blank=True
    )
    due_date_bs = serializers.CharField(
        max_length=10, validators=[validate_date_bs], allow_blank=True
    )
    ...
    purchase_details = InvPurchaseDetailCreateSerializer(many=True)
    payment_details = InvPurchasePaymentDetailCreateSerializer(
    ...
    def validate(self, attrs):
        ...
        for purchase_detail in purchase_details:
            product_variant = purchase_detail["product_variant"]
            product_category = purchase_detail["product_category"]
            calculated_net_amount = Decimal("0.00")
            calculated_discount_amount = Decimal("0.00")
            calculated_tax_amount = Decimal("0.00")

            # Validating product and product variant
            if product_variant.product.category != product_category:
                raise serializers.ValidationError(
                    {
                        "product_variant": f"product variant {product_variant.name} doesn't belongs to {product_category.name}"
                    }
                )

            # Validating gross amount
            calculated_gross_amount = Decimal(
            ...
            if purchase_detail["taxable"]:
                ...
                calculated_tax_amount = (
                    purchase_detail["tax_rate"] * partial_taxable_amount
                ) / Decimal("100")
        ...
        additional_charges = attrs.get("additional_charges", None)
    ...
    def create(self, validated_data):
        ...
        validated_data["purchase_type"] = "PURCHASE"
        purchase_no_detail = generate_unique_purchase_number(purchase_type="PURCHASE")
        ...
        # Converting Date from BS to AD
        if len(validated_data["bill_date_bs"].strip()) == 10:
            validated_data["bill_date_ad"] = date_bs_to_date_ad_converter(
                validated_data["bill_date_bs"]
            )
        else:
            validated_data["bill_date_bs"] = ""
            validated_data["bill_date_ad"] = None

        if len(validated_data["due_date_bs"].strip()) == 10:
            validated_data["due_date_ad"] = date_bs_to_date_ad_converter(
                validated_data["due_date_bs"]
            )
        else:
            validated_data["due_date_bs"] = ""
            validated_data["due_date_ad"] = None
